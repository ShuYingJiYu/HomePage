...
interface UseConfigReturn extends ConfigState {
  ...
  getApiConfig: (service: 'github' | 'gemini' | 'wordpress' | 'betterstack' | 'analytics') => Promise<any>;
  getHealthStatus: () => {
    isHealthy: boolean;
    configValid: boolean;
    errorsCount: number;
...
export function useConfig(): UseConfigReturn {
  ...
  const initializeConfig = useCallback(async () => {
    try {
      ...
      setState(prev => ({
        ...prev,
        isLoading: false,
        error: error instanceof Error ? error : new Error('Configuration initialization failed')
      }));
    }
  }, [configManager]);
  ...
  const reloadConfig = useCallback(async () => {
    try {
      ...
      setState(prev => ({
        ...prev,
        isLoading: false,
        error: error instanceof Error ? error : new Error('Configuration reload failed')
      }));
    }
  }, [configManager]);
  ...
  const isFeatureEnabled = useCallback((feature: string): boolean => {
    ...
    switch (feature) {
      ...
      case 'seo':
        return state.config.seo.enableSitemap && state.config.seo.enableRobotsTxt;
   ...
   */
  const getApiConfig = useCallback(async (service: 'github' | 'gemini' | 'wordpress' | 'betterstack' | 'analytics') => {
 ...
 */
export function useConfigSection<K extends keyof SiteConfig>(section: K): {
 ...
 */
export function useApiConfig(service: 'github' | 'gemini' | 'wordpress' | 'betterstack' | 'analytics') {
