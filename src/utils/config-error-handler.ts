...
export class ConfigErrorHandler {
  ...
  async handleError(error: ConfigError): Promise<boolean> {
    ...
    if (attempts >= 3) {
      console.error(
        `[ConfigErrorHandler] Max retry attempts reached for ${attemptKey}`
      )
      return false
    }
    ...
    switch (error.recoveryStrategy) {
      ...
      default:
        console.warn(
          `[ConfigErrorHandler] Unknown recovery strategy: ${error.recoveryStrategy}`
        )
        return false
    }
  }
  ...
  private async useCacheStrategy(error: ConfigError): Promise<boolean> {
    console.warn(
      `[ConfigErrorHandler] Using cache strategy for ${error.type}: ${error.message}`
    )
    ...
    try {
      ...
      if (cachedConfig) {
        console.info(
          '[ConfigErrorHandler] Successfully loaded configuration from cache'
        )
        return true
      }
    } catch (cacheError) {
      console.error(
        '[ConfigErrorHandler] Failed to load from cache:',
        cacheError
      )
    }
  ...
  private async skipFeatureStrategy(error: ConfigError): Promise<boolean> {
    console.warn(
      `[ConfigErrorHandler] Skipping feature due to ${error.type}: ${error.message}`
    )
  ...
  private async useFallbackStrategy(error: ConfigError): Promise<boolean> {
    console.warn(
      `[ConfigErrorHandler] Using fallback strategy for ${error.type}: ${error.message}`
    )
    ...
    try {
      ...
      if (fallbackConfig !== null) {
        console.info(
          `[ConfigErrorHandler] Applied fallback for field: ${error.field}`
        )
        return true
      }
    } catch (fallbackError) {
      console.error(
        '[ConfigErrorHandler] Fallback strategy failed:',
        fallbackError
      )
    }
   ...
   */
  private async retryWithBackoffStrategy(
    error: ConfigError,
    attempts: number
  ): Promise<boolean> {
    ...
    console.warn(
      `[ConfigErrorHandler] Retrying in ${delay}ms (attempt ${attempts + 1}) for ${error.type}: ${error.message}`
    )
  ...
  private async failBuildStrategy(error: ConfigError): Promise<boolean> {
    console.error(
      `[ConfigErrorHandler] Build failed due to ${error.type}: ${error.message}`
    )
    ...
    if (import.meta.env.DEV) {
      console.warn(
        '[ConfigErrorHandler] Development mode: continuing with errors'
      )
      return this.useFallbackStrategy(error)
    }
  ...
  private logSkippedFeature(error: ConfigError): void {
    console.info(
      `[ConfigErrorHandler] Feature skipped: ${error.field || error.type}`,
      {
  ...
  isSystemHealthy(): boolean {
    ...
    const recentCriticalErrors = this.errorLog.filter(error => {
      return (
        error.recoveryStrategy === 'fail-build' &&
...
export const createConfigError = {
  ...
  environment: (message: string, field?: string): ConfigError =>
    new ConfigError(
      message,
      ConfigErrorType.ENVIRONMENT_ERROR,
      'use-fallback',
      field
    ),
