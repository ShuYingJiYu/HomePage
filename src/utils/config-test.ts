...
export async function testConfiguration(): Promise<{
  ...
  try {
    ...
    for (const feature of features) {
      ...
      console.log(
        `${results.features[feature] ? '✅' : '❌'} ${feature}: ${results.features[feature] ? 'enabled' : 'disabled'}`
      )
    }
    ...
    for (const api of apis) {
      try {
        ...
        results.apiConnections[api] =
          !!apiConfig && Object.values(apiConfig).some(value => !!value)
        console.log(
          `${results.apiConnections[api] ? '✅' : '❌'} ${api}: ${results.apiConnections[api] ? 'configured' : 'not configured'}`
        )
      } catch (error) {
    ...
    // Generate summary
    const enabledFeatures = Object.values(results.features).filter(
      Boolean
    ).length
    const configuredApis = Object.values(results.apiConnections).filter(
      Boolean
    ).length
    const totalIssues =
      results.environment.errors.length + results.configuration.errors.length

    const success =
      results.environment.isValid &&
      results.configuration.isValid &&
      totalIssues === 0
 ...
 */
export async function testConfigSection(
  section: keyof import('../types/config').SiteConfig
): Promise<boolean> {
  try {
    const configManager = getConfigManager()
...
export function testEnvironmentVariables(): Record<string, boolean> {
  ...
  const optionalVars = ['VITE_BETTERSTACK_API_KEY', 'VITE_GA_MEASUREMENT_ID']
  ...
  requiredVars.forEach(varName => {
    ...
    console.log(
      `${results[varName] ? '✅' : '❌'} ${varName}: ${results[varName] ? 'set' : 'missing'}`
    )
  })
  ...
  optionalVars.forEach(varName => {
    ...
    results[varName] = !!value
    console.log(
      `${results[varName] ? '✅' : '⚠️'} ${varName}: ${results[varName] ? 'set' : 'optional - not set'}`
    )
  })
...
## Features
${Object.entries(testResult.results.features)
  .map(([key, value]) => `- ${key}: ${value ? '✅ Enabled' : '❌ Disabled'}`)
  .join('\n')}

## API Connections
${Object.entries(testResult.results.apiConnections)
  .map(
    ([key, value]) =>
      `- ${key}: ${value ? '✅ Configured' : '❌ Not Configured'}`
  )
  .join('\n')}
...
${
  testResult.results.environment.warnings.length > 0 ||
  testResult.results.configuration.warnings.length > 0
    ? `
### Warnings
