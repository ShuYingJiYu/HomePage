...
const ErrorRecoveryStrategySchema = z.enum(['use-cache', 'skip-feature', 'use-fallback', 'retry-backoff', 'fail-build']);
...
const SiteConfigSchema = z.object({
  site: z.object({
    ...
    description: z.object({
      zh: z.string().min(10, 'Chinese description must be at least 10 characters'),
      en: z.string().min(10, 'English description must be at least 10 characters')
    }),
    url: z.string().url('Site URL must be a valid URL'),
    defaultLanguage: LanguageSchema,
    supportedLanguages: z.array(LanguageSchema).min(1, 'At least one language must be supported'),
  ...
  async loadConfig(): Promise<SiteConfig> {
    try {
      ...
      if (!envValidation.isValid && this.options.throwOnValidationError) {
        throw new Error(`Environment validation failed: ${envValidation.errors.map(e => e.message).join(', ')}`);
      }
      ...
      if (this.options.validateOnLoad) {
        ...
        if (!this.validationResult.isValid) {
          this.log('error', 'Configuration validation failed', this.validationResult.errors);
          
          if (this.options.throwOnValidationError) {
            throw new Error(`Configuration validation failed: ${this.validationResult.errors.map(e => e.message).join(', ')}`);
          }
        }

        if (this.validationResult.warnings.length > 0) {
          this.log('warn', 'Configuration warnings', this.validationResult.warnings);
        }
      }
  ...
  validateEnvironmentVariables(): ConfigValidationResult {
    ...
    if (!import.meta.env.VITE_GA_MEASUREMENT_ID) {
      warnings.push({
        ...
        message: 'Google Analytics ID not configured - analytics will be disabled'
      });
    }
    ...
    if (!import.meta.env.VITE_BETTERSTACK_API_KEY) {
      warnings.push({
        ...
        message: 'BetterStack API key not configured - status monitoring will be disabled'
      });
    }
  ...
  private performCustomValidations(
    ...
    if (!config.analytics.googleAnalyticsId) {
      warnings.push({
        ...
        message: 'Google Analytics not configured - user tracking will be disabled'
      });
    }

    if (!config.monitoring?.betterStackApiKey && config.monitoring?.enableStatusPage) {
      warnings.push({
 ...
 */
export async function getConfig(options?: Partial<ConfigLoaderOptions>): Promise<SiteConfig> {
