...
export class ConfigManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!envValidation.isValid) {
        console.warn(
          '[ConfigManager] Environment validation issues:',
          envValidation.errors
        )

        // Handle environment errors
        for (const error of envValidation.errors) {
          const configError = createConfigError.environment(
            error.message,
            error.field
          )
      ...
      console.error(
        '[ConfigManager] Failed to initialize configuration:',
        error
      )
      ...
      if (!recovered) {
        throw new Error(
          'Configuration initialization failed and recovery was unsuccessful'
        )
      }
   ...
   */
  async getSection<K extends keyof SiteConfig>(
    section: K
